...
class EditingAdmin
{
    ...
    public function editMAdmin(array $data): bool
    {
        $checkFIO = '/^[а-яё ]+$/iu';
        $checkNum = '/^[0-9]+$/iu';
        $checkReg = '/^[а-яё.\- ]+$/iu';
        ...
        if ($_GET['user_id']) {
            ...
            if (!empty($result)) {
                $statement = $this->database->getConnection()->prepare('UPDATE login_register.military SET surname_id=:surname_id, name_id=:name_id, middle_name_id=:middle_name_id, birth_date=:birth_date, etnicity_id=:etnicity_id, military_series=:military_series, military_number=:military_number, military_issuer_id=:military_issuer_id, military_issue_date=:military_issue_date, military_commissar=:military_commissar
WHERE people_id=:people_id' );
                $statement->execute([
                    'people_id' => $result['user_id'],
